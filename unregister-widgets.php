...
class unregister_sidebar_widgets {
    ...
    public function menu() {
        add_theme_page(
            __( 'Unregister Widgets', 'widget-name' ),
            __( 'Unregister Widgets', 'widget-name' ),
            'activate_plugins',
            'unregister_widget',
            array( $this, 'form' )
    ...
    public function notice() {
        ...
        <div class="updated">
            <ul>
                <li><?php echo esc_html__( 'Saved! The widgets you chose have been hidden. :)', 'widget-name' ); ?> <a href="<?php echo admin_url( 'widgets.php' ); ?>"><?php esc_html_e( 'Widgets Page', 'widget-name' ); ?></a></li>
            </ul>
        </div>
        <?php
    }
    ...
    public function form() {
        ?>
        <div class="wrap">
            <h1><?php esc_html_e( 'Unregister Widgets', 'widget-name' ); ?></h1>
            <form id="display-form" method="post" action="">
                <h2><?php esc_html_e( 'Choose widgets to unregister', 'widget-name' ); ?></h2>
                <div class="tablenav top">
                    <div class="alignleft actions bulkactions">
                        <?php submit_button( __( 'Save Widgets', 'widget-name' ), 'button action', 'uw-submit' ); ?>
                    </div>
                    <br class="clear">
                </div>
                <table class="wp-list-table widefat plugins">
                    <thead>
                        <tr>
                            ...
                            <th scope="col"><?php esc_html_e( 'Widget Name', 'widget-name' ); ?></th>
                            <th scope="col"><?php esc_html_e( 'Description', 'widget-name' ); ?></th>
                            <th scope="col"><?php esc_html_e( 'Status', 'widget-name' ); ?></th>
                        </tr>
                    </thead>
                    <tbody id="the-list">
                        <?php
                        ...
                        foreach ( $all_wids as $key => $val ) {
                            ...
                            <td><?php echo esc_html( is_active_widget( false, false, $key, false ) ? __( 'Active', 'widget-name' ) : __( 'Inactive', 'widget-name' ) ); ?></td>                            </tr>
                            <?php
                            $num++;
                        } // end foreach.
                        ?>
